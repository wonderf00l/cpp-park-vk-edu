...
void func_() { // при перехвате искючений в подобных функциях будет происхрдить утечка ресурсов
    int* obj = new int(10);
...
// ЕСЛИ РЕАЛИЗАЦИЯ В ЗАГОЛОВОЧНОМ ФАЛЙЛЕ - ПОМЕЧАЕМ ФУНКЦИЮ/ПЕРЕМЕННУЮ С ПОМОЩЬЮ INLINE

// ИНИЦИАЛИЗАЦИЯ STATIC ПЕРЕМЕННОЙ ВНУТРИ ФУНКЦИИ(А НЕ ВНУТРИ PRIVATE СЕКЦИИ КЛАССА ЧЕРЕЗ INLINE STATIC) ГАРАНТИРУЕТ, ЧТО НА ЕЕ ИНИЦИАЛИЗАЦИЮ УСТАНОВИТСЯ ЛОК
// СРАЗУ ПОСЛЕ ТОГО, КАК ОДИН ИЗ ПОТОКОВ НАЧНЕТ ИНИЦИАЛИЗАЦЮ
int main () {
    try {
        print_smth("str");
    }
